...
while($jour<$final){
	...
	if($s==1){
		...
		couleur('gris');
		$pdf->RotatedText(15,150,utf8_decode("Syndicat National des Médecins Rhumatologues"),90);
		couleur('bleu');
		$pdf->RotatedText(21,150,"www.snmr.org",90);
	...
	if($s==4){
		...
		if(date("W",$jour)<53){
			...
			if(date("W",$jour)==1) $eme=utf8_decode('ère');
			else $eme='e';
			...
			$pdf->Cell(15,8,"Semaine", 0,0, "L", 0);
                        ...
                        $req = ("SELECT `nom`,`lieu`,`comment`,`date_debut`,`date_fin` FROM `congres` WHERE `date_debut`>='".date('Y-m-d',strtotime('-3days',$jour))."' AND `date_fin`<='".date('Y-m-d',strtotime('+3days',$jour))."' AND active='1'");
                        $res = mysql_query($req);
                        
                           /* $pdf->SetXY($congres_x,$congres_y); 
                            $pdf->SetFont('helvetica','',6);
                            $pdf->MultiCell($largeur_colone-5,8,$req);*/

                        if($res && mysql_num_rows($res)>0){
                            $congres_x = $d_l-($largeur_colone)+5;                    
                            $congres_y = 35; 

                            $pdf->SetFont('helvetica','B',7);
                            $pdf->SetXY($congres_x,$congres_y);
                            $pdf->Cell($largeur_colone-8,8,'CETTE SEMAINE', 0,0, "L", 0);
                            $congres_y+=10;
                            while($congres = mysql_fetch_array($res)){
                                ...
                                $pdf->SetXY($congres_x,$congres_y);
                                couleur('noir');
                                $pdf->MultiCell($largeur_colone-8,3,utf8_decode($congres['nom']),0,'L');
                                
                                $congres_h=$pdf->getY()+0.5;
                                $pdf->SetFont('helvetica','',7);
                                $pdf->SetXY($congres_x,$congres_h);
                                couleur('bleu');
                                $pdf->MultiCell($largeur_colone-8,3,utf8_decode($congres['date_debut']===$congres['date_fin']?'le '.human_date(strtotime($congres['date_debut'])):'du '.human_date(strtotime($congres['date_debut'])).' au '.human_date(strtotime($congres['date_fin']))), 0,"L");
                                
                                $congres_h=$pdf->getY()+0.5;
                                couleur('gris');
                                $pdf->SetFont('helvetica','',6);
                                $pdf->SetXY($congres_x,$congres_h);
                                $pdf->Cell($largeur_colone-8,2,utf8_decode($congres['lieu'].(!empty($congres['comment'])?', '.$congres['comment']:'')), 0,"L");
                                
                                $pdf->Rect($congres_x-0.5,$congres_y-1,$largeur_colone-7,$pdf->getY()-$congres_y+5,'D');
                                
                                $congres_y=$pdf->getY()+10;
                            }
                        }
		}
	}
	...
	if($s==0){
		...
		if($ro[1]==1){
			$pdf->SetFont('helvetica','',22);
			couleur('bleu');
			$pdf->RotatedText($d+22+($c*$largeur_colone),150,utf8_decode("FÉRIÉ"),45);
		}
